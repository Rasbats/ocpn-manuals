...
mkdir sources
cd sources
git clone --branch master --single-branch --depth 1 --filter=blob:none --sparse https://github.com/Rasbats/shipdriver_pi.git
cd shipdriver_pi
git sparse-checkout set manual

cd ../../sources/../sources
git clone --branch master --single-branch --depth 1 --filter=blob:none --sparse https://github.com/Rasbats/otcurrent_pi.git    
cd otcurrent_pi
git sparse-checkout set manual
